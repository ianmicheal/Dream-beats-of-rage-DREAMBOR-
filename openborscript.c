...
inline void * memsetst (void *dest, const uint8_t val, size_t len)
{
  if(!len)
  {
    return dest;
...
{
    memsetst(&spawnentry, 0, sizeof(spawnentry));//clear up the spawn entry
...
void Script_Init(Script *pscript, char *theName, char *comment, int first)
{
    if(first)
    {
        memsetst(pscript, 0, sizeof(*pscript));
        pscript->magic = script_magic;
        pscript->varlist = calloc(1, sizeof(*pscript->varlist));
        Varlist_Init(pscript->varlist, max_script_vars);
    }
...
{
    *pretvar = NULL;
    memsetst(&spawnentry, 0, sizeof(spawnentry));
